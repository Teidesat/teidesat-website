...
========================================================= */
  ...
  const floatingNavToggle = document.getElementById(
    "teidesat-floating-nav-toggle",
  );
  ...
  if (floatingNav && floatingNavToggle) {
    floatingNavToggle.onclick = function (event) {
      ...
      floatingNavToggle.setAttribute(
        "aria-expanded",
        isOpen ? "true" : "false",
      );
    };
  ...
  if (system && nodes.length) {
    ...
    function animateOrbits() {
      ...
      orbitConfigs.forEach((config) => {
        ...
        const node = system.querySelector(
          `.objective-node[data-orbit="${config.orbit}"]`,
        );
...
========================================================= */
  ...
  faqItems.forEach((item) => {
    ...
    const answer = item.querySelector(".teidesat-faq-item__answer");
    ...
    question.addEventListener("click", () => {
      ...
      if (answer) {
        ...
        // Al abrir: enfocar la respuesta para que NVDA la anuncie de inmediato
        if (isOpen) {
  ...
  if (newsSlider) {
    const viewport = newsSlider.querySelector(
      ".teidesat-news-slider__viewport",
    );
    const track = newsSlider.querySelector(".teidesat-news-slider__track");
    const dotsContainer = newsSlider.querySelector(
      ".teidesat-news-slider__dots",
    );
    const cards = Array.from(
      newsSlider.querySelectorAll(".teidesat-news-card"),
    );
